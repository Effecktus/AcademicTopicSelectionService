using System.Net;
using System.Net.Http.Json;
using AcademicTopicSelectionService.API.Authorization;
using AcademicTopicSelectionService.Application.Dictionaries.TopicCreatorTypes;
using AcademicTopicSelectionService.IntegrationTests.Infrastructure;
using FluentAssertions;

namespace AcademicTopicSelectionService.IntegrationTests.Controllers;

[Collection(DatabaseCollection.CollectionName)]
public sealed class TopicCreatorTypesControllerTests : IAsyncLifetime
{
    private const string BaseUrl = "/api/v1/topic-creator-types";

    private readonly DatabaseFixture _fixture;
    private readonly HttpClient _client;

    public TopicCreatorTypesControllerTests(DatabaseFixture fixture)
    {
        _fixture = fixture;
        _client = fixture.CreateAuthenticatedClient(AppRoles.Admin);
    }

    public async Task InitializeAsync() => await _fixture.ResetDatabaseAsync();
    public Task DisposeAsync() => Task.CompletedTask;

    // -------------------------------------------------------------------------
    // GET /api/v1/topic-creator-types
    // -------------------------------------------------------------------------

    [Fact]
    public async Task List_ReturnsEmptyPage_WhenNoTypesExist()
    {
        var response = await _client.GetAsync(BaseUrl);

        response.StatusCode.Should().Be(HttpStatusCode.OK);
        var body = await response.Content.ReadFromJsonAsync<ListResponse>();
        body!.Total.Should().Be(0);
        body.Items.Should().BeEmpty();
    }

    [Fact]
    public async Task List_ReturnsAllTypes_WhenMultipleExist()
    {
        await CreateTypeAsync("Teacher", "Научный руководитель");
        await CreateTypeAsync("Student", "Студент");

        var response = await _client.GetAsync(BaseUrl);

        response.StatusCode.Should().Be(HttpStatusCode.OK);
        var body = await response.Content.ReadFromJsonAsync<ListResponse>();
        body!.Total.Should().Be(2);
        body.Items.Should().HaveCount(2);
    }

    [Fact]
    public async Task List_ReturnsFilteredTypes_WhenSearchStringMatches()
    {
        await CreateTypeAsync("Teacher", "Научный руководитель");
        await CreateTypeAsync("Student", "Студент");

        var response = await _client.GetAsync($"{BaseUrl}?searchString=Teacher");

        response.StatusCode.Should().Be(HttpStatusCode.OK);
        var body = await response.Content.ReadFromJsonAsync<ListResponse>();
        body!.Total.Should().Be(1);
        body.Items[0].CodeName.Should().Be("Teacher");
    }

    [Fact]
    public async Task List_RespectsPageSize()
    {
        await CreateTypeAsync("Teacher", "Научный руководитель");
        await CreateTypeAsync("Student", "Студент");
        await CreateTypeAsync("DepartmentHead", "Заведующий кафедрой");

        var response = await _client.GetAsync($"{BaseUrl}?page=1&pageSize=2");

        response.StatusCode.Should().Be(HttpStatusCode.OK);
        var body = await response.Content.ReadFromJsonAsync<ListResponse>();
        body!.Total.Should().Be(3);
        body.Items.Should().HaveCount(2);
    }

    // -------------------------------------------------------------------------
    // GET /api/v1/topic-creator-types/{id}
    // -------------------------------------------------------------------------

    [Fact]
    public async Task Get_ReturnsType_WhenExists()
    {
        var created = await CreateTypeAsync("Teacher", "Научный руководитель");

        var response = await _client.GetAsync($"{BaseUrl}/{created!.Id}");

        response.StatusCode.Should().Be(HttpStatusCode.OK);
        var body = await response.Content.ReadFromJsonAsync<TopicCreatorTypeDto>();
        body!.Id.Should().Be(created.Id);
        body.CodeName.Should().Be("Teacher");
    }

    [Fact]
    public async Task Get_Returns404_WhenNotFound()
    {
        var response = await _client.GetAsync($"{BaseUrl}/{Guid.NewGuid()}");

        response.StatusCode.Should().Be(HttpStatusCode.NotFound);
    }

    // -------------------------------------------------------------------------
    // POST /api/v1/topic-creator-types
    // -------------------------------------------------------------------------

    [Fact]
    public async Task Create_Returns201_WhenDataIsValid()
    {
        var response = await _client.PostAsJsonAsync(BaseUrl, new { CodeName = "Teacher", DisplayName = "Научный руководитель" });

        response.StatusCode.Should().Be(HttpStatusCode.Created);
        var body = await response.Content.ReadFromJsonAsync<TopicCreatorTypeDto>();
        body!.CodeName.Should().Be("Teacher");
        body.DisplayName.Should().Be("Научный руководитель");
        response.Headers.Location.Should().NotBeNull();
    }

    [Theory]
    [InlineData("", "Научный руководитель")]
    [InlineData("Teacher", "")]
    public async Task Create_Returns400_WhenRequiredFieldIsEmpty(string name, string displayName)
    {
        var response = await _client.PostAsJsonAsync(BaseUrl, new { CodeName = name, DisplayName = displayName });

        response.StatusCode.Should().Be(HttpStatusCode.BadRequest);
    }

    [Fact]
    public async Task Create_Returns409_WhenNameAlreadyExists()
    {
        await CreateTypeAsync("Teacher", "Научный руководитель");

        var response = await _client.PostAsJsonAsync(BaseUrl, new { CodeName = "Teacher", DisplayName = "Другой тип" });

        response.StatusCode.Should().Be(HttpStatusCode.Conflict);
    }

    // -------------------------------------------------------------------------
    // PUT /api/v1/topic-creator-types/{id}
    // -------------------------------------------------------------------------

    [Fact]
    public async Task Update_Returns200_WhenDataIsValid()
    {
        var created = await CreateTypeAsync("Teacher", "Научный руководитель");

        var response = await _client.PutAsJsonAsync(
            $"{BaseUrl}/{created!.Id}",
            new { CodeName = "Teacher", DisplayName = "Научный руководитель (обновлён)" });

        response.StatusCode.Should().Be(HttpStatusCode.OK);
        var body = await response.Content.ReadFromJsonAsync<TopicCreatorTypeDto>();
        body!.DisplayName.Should().Be("Научный руководитель (обновлён)");
    }

    [Fact]
    public async Task Update_Returns404_WhenNotFound()
    {
        var response = await _client.PutAsJsonAsync(
            $"{BaseUrl}/{Guid.NewGuid()}",
            new { CodeName = "Teacher", DisplayName = "Научный руководитель" });

        response.StatusCode.Should().Be(HttpStatusCode.NotFound);
    }

    [Fact]
    public async Task Update_Returns409_WhenNameTakenByOtherRecord()
    {
        var type1 = await CreateTypeAsync("Teacher", "Научный руководитель");
        var type2 = await CreateTypeAsync("Student", "Студент");

        var response = await _client.PutAsJsonAsync(
            $"{BaseUrl}/{type2!.Id}",
            new { CodeName = "Teacher", DisplayName = "Студент" });

        response.StatusCode.Should().Be(HttpStatusCode.Conflict);
    }

    [Fact]
    public async Task Update_Returns400_WhenNameIsEmpty()
    {
        var created = await CreateTypeAsync("Teacher", "Научный руководитель");

        var response = await _client.PutAsJsonAsync(
            $"{BaseUrl}/{created!.Id}",
            new { CodeName = "", DisplayName = "Научный руководитель" });

        response.StatusCode.Should().Be(HttpStatusCode.BadRequest);
    }

    // -------------------------------------------------------------------------
    // PATCH /api/v1/topic-creator-types/{id}
    // -------------------------------------------------------------------------

    [Fact]
    public async Task Patch_Returns200_WhenOnlyDisplayNameProvided()
    {
        var created = await CreateTypeAsync("Teacher", "Научный руководитель");

        var response = await _client.PatchAsJsonAsync(
            $"{BaseUrl}/{created!.Id}",
            new { DisplayName = "Новый тип" });

        response.StatusCode.Should().Be(HttpStatusCode.OK);
        var body = await response.Content.ReadFromJsonAsync<TopicCreatorTypeDto>();
        body!.DisplayName.Should().Be("Новый тип");
        body.CodeName.Should().Be("Teacher");
    }

    [Fact]
    public async Task Patch_Returns400_WhenNoFieldsProvided()
    {
        var created = await CreateTypeAsync("Teacher", "Научный руководитель");

        var response = await _client.PatchAsJsonAsync(
            $"{BaseUrl}/{created!.Id}",
            new { });

        response.StatusCode.Should().Be(HttpStatusCode.BadRequest);
    }

    [Fact]
    public async Task Patch_Returns404_WhenNotFound()
    {
        var response = await _client.PatchAsJsonAsync(
            $"{BaseUrl}/{Guid.NewGuid()}",
            new { DisplayName = "Научный руководитель" });

        response.StatusCode.Should().Be(HttpStatusCode.NotFound);
    }

    // -------------------------------------------------------------------------
    // DELETE /api/v1/topic-creator-types/{id}
    // -------------------------------------------------------------------------

    [Fact]
    public async Task Delete_Returns204_WhenTypeExists()
    {
        var created = await CreateTypeAsync("Teacher", "Научный руководитель");

        var response = await _client.DeleteAsync($"{BaseUrl}/{created!.Id}");

        response.StatusCode.Should().Be(HttpStatusCode.NoContent);
    }

    [Fact]
    public async Task Delete_Returns404_WhenNotFound()
    {
        var response = await _client.DeleteAsync($"{BaseUrl}/{Guid.NewGuid()}");

        response.StatusCode.Should().Be(HttpStatusCode.NotFound);
    }

    [Fact]
    public async Task Delete_RemovesType_SoSubsequentGetReturns404()
    {
        var created = await CreateTypeAsync("Teacher", "Научный руководитель");
        await _client.DeleteAsync($"{BaseUrl}/{created!.Id}");

        var response = await _client.GetAsync($"{BaseUrl}/{created.Id}");

        response.StatusCode.Should().Be(HttpStatusCode.NotFound);
    }

    // -------------------------------------------------------------------------
    // Helpers
    // -------------------------------------------------------------------------

    private async Task<TopicCreatorTypeDto?> CreateTypeAsync(string name, string displayName)
    {
        var response = await _client.PostAsJsonAsync(BaseUrl, new { CodeName = name, DisplayName = displayName });
        response.EnsureSuccessStatusCode();
        return await response.Content.ReadFromJsonAsync<TopicCreatorTypeDto>();
    }

    private sealed record ListResponse(int Page, int PageSize, long Total, TopicCreatorTypeDto[] Items);
}
