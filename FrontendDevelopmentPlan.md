...
> **Состояние реализации (кратко, 2026-05-19):** все итерации 0–9 реализованы. Реализованы все роли: Student, Teacher, DepartmentHead, Admin. Покрыты потоки SupervisorRequests и StudentApplications, чат (polling), архив ВКР со скачиванием через presigned URL, уведомления (Inbox), аналитика и экспорт для Admin и DepartmentHead, управление пользователями и ВКР для Admin. E2E-тесты вынесены за рамки MVP.
...
### Соглашение по кнопкам (`p-button`)

Цвета кнопок управляются через `severity` — атрибут компонента `p-button`. Глобальная палитра primary-цвета задана через `definePreset` в `app.config.ts` (синий `#1a56db`) и распространяется на все компоненты PrimeNG автоматически.

#### Правила выбора `severity`

| `severity` | Внешний вид | Когда использовать |
|---|---|---|
| *(не указан)* | Синяя заливка | **Основное действие** на странице: «Сохранить», «Создать», «Принять», «Запросить», «Отправить» |
| `"secondary"` | Серая заливка | **Вторичное / нейтральное действие**: «Назад», «Вперёд» (пагинация), «Выйти» |
| `"secondary"` + `[outlined]="true"` | Серая рамка, без заливки | **Нейтральный триггер** второстепенного действия (редко; только если деструктивное действие не очевидно на данном экране) |
| `"danger"` | Красная заливка | **Деструктивное действие**: кнопка-триггер на странице («Удалить», «Отменить запрос») **и** кнопка подтверждения в диалоге |
| `"danger"` + `[outlined]="true"` | Красная рамка | **Вторичное деструктивное действие** рядом с основным (например, «Отклонить» рядом с «Принять») |

#### Паттерн «Триггер → Диалог подтверждения»

Когда действие необратимо и требует подтверждения, применять двухшаговую схему:

```
[Кнопка на странице]              [ConfirmDialog]
severity="danger"          →      acceptButtonProps: { severity: 'danger' }
label="Удалить / Отменить..."     rejectButtonProps: { severity: 'secondary' }
                                  acceptLabel: "Удалить / Отменить..."
                                  rejectLabel: "Отмена"
```

Кнопка-триггер и кнопка подтверждения оба красные — визуально сигнализируют об опасном действии на каждом из двух шагов.

#### Примеры из проекта

| Экран | Кнопка | `severity` | Обоснование |
|---|---|---|---|
| Карточка преподавателя | «Запросить научное руководство» | *(primary)* | Основное целевое действие студента |
| Детали запроса (Teacher) | «Принять» | *(primary)* | Основное позитивное действие |
| Детали запроса (Teacher) | «Отклонить» | `danger` + `outlined` | Деструктивное, второстепенное рядом с «Принять» |
| Детали запроса (Student) | «Отменить запрос» | `danger` | Деструктивное действие — триггер диалога |
| ConfirmDialog «Отмена запроса» | «Отменить запрос» | `danger` (через `acceptButtonProps`) | Подтверждение деструктивного действия |
| ConfirmDialog «Отмена запроса» | «Назад» | `secondary` (через `rejectButtonProps`) | Нейтральный отказ |
| Карточка темы | «Редактировать» | `secondary` | Нейтральное изменяющее действие |
| Карточка темы | «Удалить» | `secondary` + `outlined` | Триггер диалога подтверждения |
| ConfirmDialog «Удаление темы» | «Удалить» | `danger` (через `acceptButtonProps`) | Финальное деструктивное подтверждение |
| Пагинация (все списки) | «Назад» / «Вперёд» | `secondary` | Навигационные кнопки |
| Topbar | «Выйти» | `secondary` | Нейтральное системное действие |

> **Важно:** не использовать `severity="success"` (зелёный) — он конфликтует с основным синим цветом темы и нарушает визуальную иерархию.
...
#### Расширение итерации 4 (implementation-ready)

**Область изменений (frontend):**
- Роуты: `/supervisor-requests`, `/supervisor-requests/:id`
- Навигация: пункт меню уже добавлен, подключить рабочие страницы
- Источник создания запроса: `TeacherDetailComponent` (кнопка для `Student`)

**Техническая декомпозиция:**
1. **Контракты и API-слой**
   - Добавить `supervisor-request.models.ts` (DTO + команды + фильтры).
   - Добавить `SupervisorRequestsApiService`:
     - `GET /supervisor-requests`
     - `GET /supervisor-requests/{id}`
     - `POST /supervisor-requests`
     - `PUT /supervisor-requests/{id}/approve`
     - `PUT /supervisor-requests/{id}/reject`
     - `PUT /supervisor-requests/{id}/cancel`

2. **Страница списка (`SupervisorRequestsListComponent`)**
...
3. **Страница деталей (`SupervisorRequestDetailComponent`)**
   ...
   - Действия по правилам:
     - Student + `Pending` → «Отменить».
     - Teacher + `Pending` → «Принять», «Отклонить».
   - Для отклонения: диалог с обязательным комментарием.
   - После действия: обновление данных детали + переход/синхронизация списка.

4. **Интеграция в `TeacherDetailComponent`**
   - Кнопка «Запросить научное руководство» только для `Student`.
   - Блокировка кнопки во время запроса.
   - Обработка конфликтов (`409`) с понятным сообщением.

5. **Доступ и безопасность на фронте**
   - Маршруты `/supervisor-requests*` ограничить ролями `Student` и `Teacher`.
   - UI-ограничения продублировать в компонентах (не только через guard).

**Definition of Done (DoD):**
- [ ] UI-поток полностью покрывает create/list/detail/approve/reject/cancel
- [ ] Для reject нельзя отправить пустой комментарий
- [ ] Ошибки API отображаются понятными сообщениями
- [ ] Роуты защищены по ролям (`Student`, `Teacher`)
