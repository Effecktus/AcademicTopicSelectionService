...
- Поток заявки на тему: после одобрения научруком (`PUT .../applications/{id}/approve`) заявка сразу переходит к заведующему; отдельный шаг `submit-to-department-head` отключён в бизнес-логике (см. `v1.applications.md`).
