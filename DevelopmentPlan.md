...
## Актуальный статус реализации (на 2026-04-27)
...
| Этап 3: Backend функциональность (итерации 3а/3б) | ✅ Выполнено | 2026-04-27 | Потоки `SupervisorRequests` + `StudentApplications`; фильтры списков тем и заявок научрука по `createdFromUtc`/`createdToUtc`; unit/integration тесты (в т.ч. сценарии дат) |
| Этап 3: Chat/VKR/Notifications | ✅ Частично | 2026-04-27 | Чат, архив ВКР, уведомления на backend реализованы; детали — `BackendDevelopmentPlan.md` |
| Этап 4: Frontend | 🔄 В работе | 2026-04-27 | Angular 20 SPA: списки тем, преподавателей, заявок научрука; сортировка, пагинация, фильтр по дате создания; см. `FrontendDevelopmentPlan.md` |
| Этап 5: Тестирование и доработка | 🔄 Частично | 2026-04-27 | Backend unit/integration; frontend unit (Karma) для утилит и ключевых сервисов; E2E по плану |
...
4. Научрук одобряет/отклоняет заявку на тему
5. При одобрении заявка отправляется заведующему кафедрой
...
- `PUT /api/v1/applications/{id}/submit-to-department-head` — передача заведующему
