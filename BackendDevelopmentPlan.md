# План разработки Backend (ASP.NET Core 10) для проекта «DirectoryOfGraduates»
...
- Тестовые данные: `infra/db/test_data/98_seed_test_data.sql`
- Compose‑файлы общего стека: `infra/docker/compose.*.yml`
- Каркас backend: `backend/src/*` и Solution `backend/DirectoryOfGraduates.slnx`
- Минимальные проверки здоровья API:
  - `GET /health`
  - `GET /health/db` (подключение к PostgreSQL)
- Версионирование API в URL: `/api/v1/...`
- Эталонный CRUD: `UserRoles` (`/api/v1/user-roles`)
  - Swagger: `http://localhost:5000/swagger`
  - Markdown-документация: `docs/api/v1.user-roles.md`
...
- модель (`DbContext` + сущности) генерируется scaffold’ом из реальной БД в `DirectoryOfGraduates.Infrastructure`;
- миграции EF Core пока **не используем**, чтобы не вести 2 параллельных источника истины.

## 2) Архитектура решения и слоёв (как в общем плане)

Целевая структура (совпадает по смыслу с `DevelopmentPlan.md` и уже создана):

```
backend/
  DirectoryOfGraduates.slnx
  Dockerfile
  src/
    DirectoryOfGraduates.API/           # Web API (Controllers, swagger, health)
    DirectoryOfGraduates.Application/   # use-cases/сервисы, DTO/контракты, абстракции репозиториев
    DirectoryOfGraduates.Infrastructure/# EF Core (scaffold DbContext+Entities), репозитории, внешние интеграции
    DirectoryOfGraduates.Shared/        # общие константы/хелперы/extension методы
```

### Правило зависимостей (чтобы не было хаоса)
