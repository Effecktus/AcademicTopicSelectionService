...
- Каркас backend: `backend/src/*` и Solution `backend/AcademicTopicSelectionService.slnx`
...
backend/
  ...
  Dockerfile
  src/
    AcademicTopicSelectionService.API/           # Web API (Controllers, swagger, health)
    AcademicTopicSelectionService.Application/   # use-cases/сервисы, DTO/контракты, абстракции репозиториев
    AcademicTopicSelectionService.Infrastructure/# EF Core (scaffold DbContext+Entities), репозитории, внешние интеграции
    AcademicTopicSelectionService.Shared/        # общие константы/хелперы/extension методы
```
