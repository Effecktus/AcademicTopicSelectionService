...
-- Зависит от: Students, Teachers, StudentApplications, GraduateWorkStatuses
...
CREATE TABLE "GraduateWorks" (
    ...
    CONSTRAINT "CK_GraduateWorks_Grade_Range" CHECK ("Grade" IS NULL OR "Grade" BETWEEN 0 AND 100),
    CONSTRAINT "CK_GraduateWorks_CommissionMembers_NotEmpty" CHECK ("CommissionMembers" IS NULL OR length(btrim("CommissionMembers")) > 0),
    ...
    CONSTRAINT "FK_GraduateWorks_GraduateWorkStatuses"
        FOREIGN KEY ("StatusId")
        REFERENCES "GraduateWorkStatuses"("Id")
        ON DELETE RESTRICT
        ON UPDATE CASCADE,
...
COMMENT ON COLUMN "GraduateWorks"."StatusId"             IS 'Идентификатор статуса ВКР (внешний ключ к таблице GraduateWorkStatuses)';
...
COMMENT ON COLUMN "GraduateWorks"."Grade"                IS 'Оценка за работу (от 0 до 100 баллов); NULL до заполнения администратором после защиты';
COMMENT ON COLUMN "GraduateWorks"."CommissionMembers"    IS 'Состав комиссии, оценивавшей работу (текстовое описание); NULL до заполнения администратором после защиты';
